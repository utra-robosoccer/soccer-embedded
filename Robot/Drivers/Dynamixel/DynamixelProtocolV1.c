...
uint16_t Dynamixel_DataReader_DMA(Dynamixel_HandleTypeDef* hdynamixel, uint8_t readAddr, uint8_t readLength){
	...
	// Transmit
	HAL_UART_Transmit_DMA(hdynamixel -> _UART_Handle, arrTransmit[ID], 8, TRANSMIT_TIMEOUT);
	...
	// Call appropriate UART receive function depending on if 1 or 2 bytes are to be read
	if(readLength == 1){
		HAL_UART_Receive_DMA(hdynamixel -> _UART_Handle, arrReceive[ID], 7, RECEIVE_TIMEOUT);
	}
	else{
		HAL_UART_Receive_DMA(hdynamixel -> _UART_Handle, arrReceive[ID], 8, RECEIVE_TIMEOUT);
	}
